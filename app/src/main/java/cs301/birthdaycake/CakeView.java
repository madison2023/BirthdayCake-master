...
public class CakeView extends SurfaceView {
    ...
    public static final float candleWidth = 75.0f;
    ...
    public CakeView(Context context, AttributeSet attrs) {
        ...
        balloonPaint.setColor(0xFF335FFF);
        balloonPaint.setStyle(Paint.Style.FILL);
        balloonString.setColor(Color.BLACK);
        balloonString.setStrokeWidth(3);
    ...
    public void drawBalloon(Canvas canvas, float cx, float cy) {
        canvas.drawOval(cx-40,cy-50,cx+40, cy+50, balloonPaint);
        canvas.drawLine(cx,cy+50,cx,cy+120,balloonString);
    }
    /**
    ...
    public void onDraw(Canvas canvas) {
        ...
        int candlesNum = myCakeModel.numCandles;
        int i;
        for(i = 1; i <= candlesNum; i++) {
            drawCandle(canvas, (cakeLeft + (((cakeWidth / (candlesNum + 1)) * i)) - (candleWidth / 2)) , cakeTop);
        }
...
//        drawCandle(canvas, cakeLeft + 2*(cakeWidth/3) - candleWidth/2, cakeTop);
    ...
    public CakeModel getMyCakeModel() {
        return myCakeModel; //is this the right thing to return
    }
